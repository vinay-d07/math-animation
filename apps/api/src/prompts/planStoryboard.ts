...
const MODE_SYSTEM_PROMPTS: Record<VideoGenerationMode, string> = {
  SCENES: `${SHARED_INTRO}
...
export function getPlanningSystemPrompt(mode: VideoGenerationMode): string {
  return MODE_SYSTEM_PROMPTS[mode];
}

export function buildStoryboardPrompt(topic: string): string {
  return `Topic: ${topic}`;
}

/**
 * Only sceneClassName + narration are echoed back (not visualIntent/
 * explanation/title) — that's all the model needs to avoid repeating itself
 * and extend coherently, and on an 8K TPM free-tier budget the full
 * storyboard JSON is expensive dead weight.
 */
export function buildExpansionPrompt(
  topic: string,
  priorScenes: { sceneClassName: string; narration: string }[],
  currentWords: number,
  minWords: number,
  targetWords: number,
  maxScenes: number
): string {
  const priorNarration = priorScenes
...
scenes, up to ${maxScenes} total. Return the FULL revised storyboard (all fields for
